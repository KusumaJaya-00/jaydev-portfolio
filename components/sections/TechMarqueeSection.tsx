...
export function TechMarqueeSection() {
  return (
    <section className="relative pt-4 pb-10 border-y border-border overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="marquee-wrap animate-marquee will-change-transform">
          {[0, 1, 2, 3].map((copy) => (
            <div key={copy} className="flex items-center shrink-0" aria-hidden={copy !== 0}>
              {TECH.map((tech) => (
                <div
                  key={`${copy}-${tech}`}
                  className="flex items-center gap-2 px-8 sm:px-10 text-lg sm:text-xl font-semibold text-muted-foreground whitespace-nowrap"
                >
