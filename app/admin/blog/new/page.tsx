...
export default function NewPostPage() {
 ...
 const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  // Post baru ditaruh di urutan paling akhir.
  let sortOrder = 0
  const { data: maxRows } = await supabase.from('posts').select('sort_order').order('sort_order', { ascending: false }).limit(1)
 ...
 return (
  <div>
   ...
   <form onSubmit={handleSubmit} className="notch bg-card border border-border space-y-5 p-6 md:p-8">
    ...
    <ImageUpload name="featured_image" label="Image" folder="posts" />
    <GalleryUpload name="gallery" label="Gallery Images" folder="posts" />
